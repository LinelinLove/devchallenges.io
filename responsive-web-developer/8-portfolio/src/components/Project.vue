      ...
      >
        ...
        <div class="flex flex-row gap-x-4">
          <button
            ...
            :key="tag"
            class="text-p text-grey-primary px-5 py-2 border rounded-xl border-grey-primary hover:text-p-blue hover:border-p-blue active:bg-p-blue active:text-white"
            @click="toggleTag(tag)"
...
export default {
  ...
  methods: {
    ...
    getAllTags() {
      const tags = new Set(); // Utilisation d'un ensemble pour éviter les doublons
      this.datas.data.projects.forEach((project) => {
        project.tag.forEach((tag) => {
          tags.add(tag);
        });
      });
      return Array.from(tags); // Convertir l'ensemble en tableau
    },
  },
};
</script>
